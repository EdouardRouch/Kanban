    ...
    if (!empty($data->id)) {
        ...
        if ($kanban->is_owner($_SESSION("username"))) {
            try {
                ...
                echo json_encode(new ResponseBody("Kanban supprimé avec succès."));
...
// si l'utilisateur n'est pas connecté, il ne peut pas supprimer de Kanban
else {
    http_response_code(401);
    echo json_encode(new ResponseBody("Vous devez être connecté pour supprimer un Kanban."));
}
